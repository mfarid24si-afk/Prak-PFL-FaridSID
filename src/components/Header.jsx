      ...
      <div id="icons-container" className="flex items-center gap-6">
        ...
        <div id="profile-container" className="flex items-center gap-3">
          <div className="text-right">
            <span id="profile-text" className="text-sm font-bold text-gray-800 block">
              Hai, <b className="font-extrabold text-green-600">{profile?.full_name || "User"}</b>
            </span>
            <span className="text-[10px] text-gray-400 capitalize">{profile?.role} • {profile?.points || 0} pts • {profile?.tier}</span>
          </div>
          <img
            src={profile?.avatar_url || "public/img/ayam.png"}
