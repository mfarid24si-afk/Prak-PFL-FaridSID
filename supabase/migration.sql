-- =============================================================
-- MIGRATION: Full System Setup
-- Tables, Functions, Triggers, RLS, and Seed Data
...
-- 3. CUSTOMERS TABLE (walk-in / non-login customers)
...
-- 7. TRIGGER: Generate order_no (ORD-YYYYMMDD-XXXX)
...
-- 8. TRIGGER: Recalc order totals when items change
...
-- 9. TRIGGER: Stock management (+/-) on order items
...
-- 10. TRIGGER: Award/revoke loyalty points on order status change
...
BEGIN
  -- Only react on actual status transitions
  ...
  -- Revoke points when status changes FROM 'completed' (e.g. cancelled)
  IF OLD.status = 'completed' AND NEW.status != 'completed' THEN
    IF OLD.user_id IS NOT NULL THEN
      UPDATE public.profiles
      SET
        points = GREATEST(0, points - OLD.points_awarded),
        updated_at = now()
...
-- 11. FUNCTION: Auto-update tier based on points
...
-- PROFILES RLS
-- Admin: read/write all. Member/Guest: read/write own (except role/points/tier)
...
CREATE POLICY profiles_self_update ON public.profiles
  ...
  WITH CHECK (
    ...
    AND points IS NOT DISTINCT FROM OLD.points
    AND tier IS NOT DISTINCT FROM OLD.tier
  );

-- CUSTOMERS RLS (admin only)
CREATE POLICY customers_admin_all ON public.customers
  FOR ALL USING (public.is_admin()) WITH CHECK (public.is_admin());

-- PRODUCTS RLS
-- Admin: CRUD. All authenticated + anon: read active.
...
-- ORDERS RLS
-- Admin: CRUD all. Member: create own, read own, cancel own when pending.
...
-- ORDER ITEMS RLS
-- Admin: CRUD all.
-- Member: CRUD on items belonging to own orders that are pending.
...
-- 13. SEED DATA
-- Admin user: you need to create this user via Supabase Auth first, then run:
-- UPDATE public.profiles SET role = 'admin' WHERE email = 'admin@example.com';
-- Or create the user with raw_user_meta_data: {"role":"admin","full_name":"Admin"}
...
-- 2. Run: UPDATE public.profiles SET role = 'admin' WHERE email = '<admin-email>';
-- =============================================================
